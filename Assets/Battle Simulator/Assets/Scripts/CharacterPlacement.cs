...
public enum TroopType { Melee, Ranged, Cavalry, Heavy, Special, Epic }
...
public class CharacterPlacement : MonoBehaviour {
	...
	private TroopType currentFilter = TroopType.Melee;
	
	private bool erasing;
	private Color eraseStartColor;
	private int coins;
	private int maxPlaceTroops;
	...
	private int currentPlacedUnits = 0;
	
	private bool mobile;
	private int gridSize;	
	private bool champSelected = false;
	...
	void Awake(){
		...
		// BƯỚC 1: KIỂM TRA LEVEL VÀ SINH RA MAP 
		int currentLevel = PlayerPrefs.GetInt("level");
		if(currentLevel >= levelData.levels.Count){
			SceneManager.LoadScene("End screen");
			return; // Thoát hàm luôn nếu hết level
		}
		
		// Đổi text giao diện
		levelInfo.text = "Level " + (currentLevel + 1) + " - " + levelData.levels[currentLevel].mapName;

		// Lấy prefab map của level hiện tại
		GameObject prefabToSpawn = levelData.levels[currentLevel].mapPrefab;
		if (prefabToSpawn != null) {
			GameObject spawnedMap = Instantiate(prefabToSpawn, prefabToSpawn.transform.position, Quaternion.identity);
		}

		// BƯỚC 2: TÌM ENEMY ARMY BÊN TRONG MAP VỪA ĐƯỢC SINH RA
		...
		// Nếu dùng grid, vẽ viền 3D và lưới 2D
		if(levelData.grid){
			createBorder();
			initializeGrid();
		}
	...
	public void filterTroops(int filterIndex)
	{
		currentFilter = (TroopType)filterIndex;

		for (int i = 0; i < troops.Count; i++)
		{
			bool show = troops[i].troopType == currentFilter;
			if (troops[i].button != null) {
				troops[i].button.SetActive(show);
			}
		}
	}
	...
	void Start(){
		...
		maxPlaceTroops = levelData.levels[PlayerPrefs.GetInt("level")].quanityLimit;
		QuanityLimitedText.text = currentPlacedUnits + "/" + maxPlaceTroops;
	...
	void Update(){
		...
		bool isCameraReady = (camMovement == null) || camMovement.IsCameraFree();
		if(currentDemoCharacter){
			...
			if(!erasing){	
				...
				if(Input.GetMouseButton(0) && position.x < 0 && isCameraReady)
				...
				if(unitsInRange(position) != null || position.x > 0 || troops[selected].troopCosts > coins || Vector3.Distance(Camera.main.transform.position, position) > levelData.placeRange || !withinGrid(position) || !isCameraReady){
			...
			else if(Input.GetMouseButton(0) || erasingUsingKey && isCameraReady){
	...
	public void placeUnit(Vector3 position, bool placingGridCell){
		...
		if(canPlace(position, placingGridCell) && currentPlacedUnits < levelData.levels[PlayerPrefs.GetInt("level")].quanityLimit){
			currentPlacedUnits++;
			QuanityLimitedText.text = currentPlacedUnits + "/" + maxPlaceTroops;
			Debug.Log("currentPlaceUnits: " + currentPlacedUnits);
	...
	IEnumerator addCharacterButtons(){
		...
		for(int i = 0; i < troops.Count; i++){
			...
			//set button outline
			newButton.GetComponent<UnityEngine.UI.Outline>().effectColor = levelData.buttonHighlight;
		...
		filterTroops((int)TroopType.Melee);
	...
	public void setStats(int index){
		...
		if(statsAttackSpeed != null)
        statsAttackSpeed.text = unit.attackSpeed + "";
	}
	...
	public void reloadScene(){
		...
		Time.timeScale = 1; //reset the timescale after win/lose
	}
