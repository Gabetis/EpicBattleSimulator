...
public class Menu : MonoBehaviour {
	...
	void Start(){
		...
		Time.timeScale = 1; //reset timescale when back to menu
	}
	...
	void addLevels(){		
		...
		for(int i = 0; i < levelData.levels.Count; i++){
			//get the level scene
			string sceneName = levelData.levels[i].mapName;
	...
	IEnumerator level(int index){
		...
		SceneManager.LoadScene(levelData.levels[index].mapName);
	}
	...
	string getSceneName(Transform levelPage){
		...
		return levelData.levels[firstLevel].mapName;
	}
