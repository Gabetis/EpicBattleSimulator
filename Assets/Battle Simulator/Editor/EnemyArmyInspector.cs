...
public class EnemyArmyInspector : Editor{
	...
	void OnEnable(){
		...
		levelList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) => {
			EditorGUI.LabelField(new Rect(rect.x, rect.y + 2, Screen.width, EditorGUIUtility.singleLineHeight), "Level " + (index + 1) + ": " + levelData.levels[index].mapName, EditorStyles.largeLabel);
		...
		levelList.onAddCallback = (ReorderableList l) => {  
			if(levelData.levels.Count > 0){
				levelData.levels.Add(new EnemyArmyLevel {mapPrefab = levelData.levels[levelData.levels.Count - 1].mapPrefab});
			}
			else{
				levelData.levels.Add(new EnemyArmyLevel {mapPrefab = null});
			}
		};
	...
	if(toolbarSelected == 0){
		...
		if(levelData.levels.Count > 0){
			...
			if(activeElement < levelData.levels.Count)
				levelData.levels[activeElement].mapName = EditorGUILayout.TextField("Map Name", levelData.levels[activeElement].mapName);
				
				levelData.levels[activeElement].mapPrefab = (GameObject)EditorGUILayout.ObjectField("Map Prefab", levelData.levels[activeElement].mapPrefab, typeof(GameObject), false);		
